    ...
    var body: some View {
        List {
            ...
            ForEach(visible) { receipt in
                ...
                // Hold to pop the full receipt image.
                .contextMenu {
                    if receipt.matchStatus == .unmatched {
                        Button {
                            receipt.matchStatus = .noMatchExpected
                            try? modelContext.save()
                        } label: {
                            Label("Mark as cash purchase", systemImage: "banknote")
                        }
                    }
                } preview: {
                    if let image = ReceiptCaptureService.loadImage(reference: receipt.imageReference) {
                        ...
                        ReceiptRow(receipt: receipt).padding(20).frame(width: 320)
                    }
                }
                .glassListRow()
            }
            ...
            if visible.isEmpty && !isProcessing {
                EmptyStateView(
                    ...
                    message: "Scan a receipt and Polaris extracts the merchant, total, tip, and items — then matches it to the card charge.",
